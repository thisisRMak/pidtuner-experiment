...
def _new_judge_session(candidate_clients, judge_client) -> JudgeSession:
    candidate_sessions = [
        (label, LQGSession(client, lqg_tool=LQG_TOOL))
...
def main():
    ...
    print(f"PIDTuner LLM judge (LQR/LQG track). Candidates: "
          f"{', '.join(label for label, _ in candidate_clients)}. Judge: {_label(*resolved_judge)}.")
    print("Tell me which preset plant you're working with and what matters most to you. Type "
