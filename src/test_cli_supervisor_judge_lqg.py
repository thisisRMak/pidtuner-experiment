"""Unit tests for cli_supervisor_judge_lqg.py's provider/model/key-
resolution plumbing and cross-flag validation -- no live API required.
Mirrors test_cli_supervisor_judge_pid.py's style/conventions exactly (this
is the MIMO/LQG track counterpart -- same plumbing, different candidate
session class and judge prompt).

Run with:
    pytest src/test_cli_supervisor_judge_lqg.py
...
cli_supervisor_judge_pid.py's own, just duplicated per that module's own
convention), the "at least 2 candidates" / "judge must differ from every
candidate" cross-flag invariants, client dispatch, and that
_new_judge_session wires up LQGSession candidates and the LQG judge prompt
rather than PID/SISO's. The REPL loop in main() itself is untested here,
same as cli_supervisor_judge_pid.py's own test file -- see
test_supervisor_session_judge_pid.py for JudgeSession's own orchestration
coverage, reused as-is by this CLI.
"""
...
class TestResolveAnthropicKey(unittest.TestCase):
    def test_explicit_key_wins_without_consulting_env(self):
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "sk-env"}), \
             patch("cli_supervisor_judge_lqg.load_dotenv") as mock_load_dotenv:
    ...
    def test_falls_back_to_env_var_when_no_explicit_key(self):
        with patch.dict("os.environ", {"ANTHROPIC_API_KEY": "sk-env"}), \
             patch("cli_supervisor_judge_lqg.load_dotenv"):
...
class TestResolveKeyDispatch(unittest.TestCase):
    ...
    def test_dispatches_to_anthropic_resolver(self):
        with patch.dict("os.environ", {}, clear=True), patch("cli_supervisor_judge_lqg.load_dotenv"):
...
class TestNewJudgeSession(unittest.TestCase):
    ...
    def test_candidates_are_lqg_sessions_not_pid_sessions(self):
        """The one substantive difference from the SISO/PID CLI's own
        _new_judge_session: candidates must be LQGSession instances (built
        with the LQG benchmark tool), not supervisor_session_pid.Session."""
        candidate_clients = [
            ("anthropic:claude-haiku-4-5", AnthropicClient(api_key="sk-a", model=ANTHROPIC_DEFAULT_MODEL)),
            ("openai:gpt-5.6-luna", OpenAIClient(api_key="sk-b", model=OPENAI_DEFAULT_MODEL)),
        ]
        judge_client = GeminiClient(api_key="sk-c", model=GEMINI_DEFAULT_MODEL)
        session = cli._new_judge_session(candidate_clients, judge_client)
        for _, candidate_session in session.candidates:
            self.assertIsInstance(candidate_session, LQGSession)

    def test_judge_system_prompt_is_the_lqg_one(self):
        """The other substantive difference: the judge must be given the
        LQG-flavored judge prompt, not JudgeSession's PID-flavored
        default."""
        candidate_clients = [
            ("anthropic:claude-haiku-4-5", AnthropicClient(api_key="sk-a", model=ANTHROPIC_DEFAULT_MODEL)),
            ("openai:gpt-5.6-luna", OpenAIClient(api_key="sk-b", model=OPENAI_DEFAULT_MODEL)),
        ...
        judge_client = GeminiClient(api_key="sk-c", model=GEMINI_DEFAULT_MODEL)
        session = cli._new_judge_session(candidate_clients, judge_client)
        self.assertEqual(session.judge_system_prompt, JUDGE_SYSTEM_PROMPT_LQG)
