...
class LQGSession:
    ...
    def _wrap_benchmark(self, fn):
        ...
        "_sim_rows" (each row's .sim intact, not JSON-safe),
        "_custom_plant_literals" (present only for a custom plant), and
        "_four_curve" (present only when am_diag was supplied -- see
        run_lqg_benchmark) off the result before it goes anywhere near
        json.dumps/the model. capture_plots=False (the default) asks for
        return_sim=False instead, skipping that simulation work entirely
        for a caller (e.g. cli_supervisor_lqg.py) with no drain step to
        use it.
        ...
        def _wrapped(**kwargs):
            ...
            four_curve = result.pop("_four_curve", None)
