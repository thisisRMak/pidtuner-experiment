#!/usr/bin/env bash
# Demonstrates cli_pid.py: the white-box tuner, which knows the plant's true
...
python3 cli_pid.py --plant "$PLANT" --method all --plot "$OUT/compare_all.png" --json > /dev/null
echo "wrote $OUT/compare_all.png"

echo
echo "### 8. Plant with explicit dead time (delay-dominant, Cohen-Coon shines) ###"
python3 cli_pid.py --plant "2/(5s+1)" --L 3 --method cohen_coon
