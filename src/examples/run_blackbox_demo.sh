#!/usr/bin/env bash
# Demonstrates the black-box pipeline: cli_pid.py exports a Signal (entity A,
# the "plant owner") to a .npz file; cli_pid_blackbox.py (entity C, the
# "black-box tuner") consumes only that file and never sees --plant/--L.
# This mirrors the process-boundary isolation enforced by pid_blackbox.py.
set -euo pipefail
cd "$(dirname "$0")/.."
OUT=examples/out
mkdir -p "$OUT"
...
python3 cli_pid.py --plant "$PLANT" --L "$L" \
...
python3 cli_pid_blackbox.py --in-signal-step "$OUT/step.npz"

echo
echo "### 4. Entity C: add the relay signal too, JSON output, saved to file ###"
python3 cli_pid_blackbox.py \
...
python3 cli_pid_blackbox.py --in-signal-relay "$OUT/relay.npz"
