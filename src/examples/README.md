...
script `cd`s to `src/` itself and writes artifacts to `examples/out/`
...
- **`run_whitebox_demo.sh`** — `cli_pid.py`, the white-box tuner. Runs
...
- **`run_blackbox_demo.sh`** — the two-process black-box pipeline.
  `cli_pid.py --gen-signal` (entity A) publishes step/relay test signals to
  `.npz` files *without* exposing the plant's transfer function; then
  `cli_pid_blackbox.py` (entity C) identifies a model and tunes all 9 methods
  from those signal files alone.

- **`run_supervisor_demo.sh`** — `cli_supervisor_pid.py`, the conversational
